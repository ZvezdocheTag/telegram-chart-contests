...
(function () {
  ...
  var chart = {
    ...
    init: function init() {
      ...
      var dataReq = Object.keys(getDataFormat).map(function (key) {
        return getDataFormat[key];
      });
      ...
      try {
        ...
        for (var _iterator = dataReq[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          _loop();
        }
      } catch (err) {
  ...
  var ChartRoot = {
    ...
    setStyleMode: function setStyleMode(params) {
      ...
      Object.keys(_this2.state.chart).map(function (key) {
        return _this2.state.chart[key];
    ...
    init: function init(id, main, data, title, colorType) {
      ...
      Object.entries(data.names).forEach(function (_ref) {
        ...
        if (!_this3.state.ineracted[idAttr]) {
          _this3.state.ineracted[idAttr] = {};
        }

        _this3.state.ineracted[idAttr][key] = {
          name: name,
          active: false
        };
      });
      ...
      controls.childNodes.forEach(function (child) {
          ...
          function check() {
            ...
            for (var i = 0; i < controls.children.length; i += 1) {
              ...
              if (!_this.isSameNode(childOther)) {
                var btnId = childOther.dataset.toggleBtn;
        ...
        child.addEventListener('click', function (e) {
          ...
          var _self$state$ranges$id2 = _slicedToArray(self.state.ranges[idAttr], 2),
              min = _self$state$ranges$id2[0],
              max = _self$state$ranges$id2[1];
      ...
      function Tooltip(svg, colr, chartTypes) {
        ...
        function moveMouse(e) {
          ...
          var mapped = getFirstRange.map(function (d, i) {
            return _objectSpread({}, d, {
              idx: i
            });
          });

          var _mapped$filter$slice = mapped.filter(function (item) {
            return item.x < offsetX;
          }).slice(-1),
              _mapped$filter$slice2 = _slicedToArray(_mapped$filter$slice, 1),
              currentTooltipPos = _mapped$filter$slice2[0];

          var currentCoords = [];
          currentChart.forEach(function (item) {
        ...
        function mouseLeave(e) {
          line = null;
          var list = tooltip.querySelector('.tooltip-list');
          svg.querySelectorAll('circle').forEach(function (item) {
            item.remove();
          });
          list.remove();
  ...
  function widthAnimationA(start, ctx) {
    return function step(timestamp) {
      ...
      var limit = 280;
      var xValue = Math.min(progress / 10, limit);
  ...
  function processCoords(w, h, ranges, lines, status) {
    ...
    var commonMinMax = getRangeMinMax(merged);
  ...
  var normilizeColumns = function normilizeColumns(columns) {
    return columns.reduce(function (curr, _ref9) {
      ...
      return _objectSpread({}, curr, _defineProperty({}, key, values));
    }, {});
  };
  ...
  function scaleLiniar(_ref11, _ref12) {
  ...
  function scaleTime(_ref15, _ref16) {
  ...
  function () {
    ...
    _createClass(Magnifier, [{
      ...
      value: function resizeLeft(width, resize) {
        ...
        setTimeout(function () {
          _this4.actionResize.update(resize, width + resize, _this4.interacted);
        }, 0);
      }
    }, {
      key: "resizeRight",
      value: function resizeRight(width, left, shadow, axis) {
        ...
        setTimeout(function () {
          _this5.actionResize.update(left, axis, _this5.interacted);
        }, 0);
      }
    }, {
      key: "dragCenter",
      value: function dragCenter(l, r, width) {
        ...
        setTimeout(function () {
          _this6.actionResize.update(l, l + width, _this6.interacted);
        }, 0);
      }
    }, {
      key: "resizeStart",
      value: function resizeStart(e) {
        ...
        var leftWrapper = getCoords(this.wrapper).left;
        var handlersWidth = 9;
        var pageX = e.pageX - leftWrapper;
        ...
        var resize = function resize(ec) {
          var resizePageX = ec.pageX - leftWrapper;
          ...
          if (side === 'right') {
            if (maxRight >= 0 && containerWidth - mR > 0 && elW > 30) {
              _this7.resizeRight(elW, offset, shD, mR);
            }
          }
          ...
          if (side === 'left') {
            if (finishLeft < containerWidth && wwC > 30) {
              _this7.resizeLeft(wwC, maxLeft);
            }
          }

          if (side === 'center') {
            if (l > handlersWidth && containerWidth - mR > 0) {
              _this7.dragCenter(l, r, width);
            }
          }
        };
  ...
  function ChartTemplate(name, chart, _ref19) {
    var w = _ref19.w,
        h = _ref19.h,
        mW = _ref19.mW,
        mH = _ref19.mH,
        colors = _ref19.colors,
        pallet = _ref19.pallet,
        title = _ref19.title;
